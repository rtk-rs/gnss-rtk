...
mod test {
    ...
    use nalgebra::{DVector, DimName, U4, U7};
    ...
    #[test]
    fn state_u7_from_reference_apriori_position() {
        let earth_frame = build_earth_frame();
        let apriori = build_reference_apriori();

        let initial_state = State::<U7>::from_apriori(&apriori).unwrap_or_else(|e| {
            panic!(
                "Failed to build initial state from reference apriori: {}",
                e
            )
        ...
        assert_eq!(
            ...
            (0.0, 0.0, 0.0),
            "dynamics should not be initialized!",
        );
        ...
        let null_dx = DVector::zeros(U7::USIZE);
        let new_t = state.t;
        ...
        let mut dx = DVector::zeros(U7::USIZE);

        dx[0] = 1.0;
        dx[1] = 2.0;
        dx[2] = 3.0;

        dx[Navigation::<U7>::clock_index()] = 4.0;
        ...
        assert_eq!(
            ...
            (0.0, 0.0, 0.0),
            "dynamics should have been preserved!"
        );
    ...
    #[test]
    fn state_u7_from_reference_orbital_state() {
        let orbit = build_reference_orbit();

        let state = State::<U7>::from_orbit(&orbit).unwrap_or_else(|e| {
            panic!("Failed to build initial state from reference orbit: {}", e)
        });
